...
/// PROBDENSITY over a cone, at the demo depth. Thin wrapper over the
/// shared [`boom_gw::skymap_synth::build_uniform_cone_skymap`] so the
/// loader, integration tests, and unit tests use one implementation.
fn build_synthetic_prob_density_skymap(ra_deg: f64, dec_deg: f64, radius_deg: f64) -> Vec<u8> {
    boom_gw::skymap_synth::build_uniform_cone_skymap(ra_deg, dec_deg, radius_deg, DEMO_MOC_DEPTH)
}
